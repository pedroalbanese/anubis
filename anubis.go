...
// constants in case we decide to support larger key sizes in the futuree
const (
	_MIN_N        = 4
	_MAX_N        = 10
...
type Cipher struct {
	keyBits     int
	r           int
	roundKeyEnc [_MAX_ROUNDS + 1][4]uint32
...
func crypt(plaintext []byte, ciphertext []byte, roundKey [_MAX_ROUNDS + 1][4]uint32, R int) {
	...
	// last round:
	inter[0] =
		(_T0[(state[0]>>24)] & 0xff000000) ^
			(_T1[(state[1]>>24)] & 0x00ff0000) ^
			(_T2[(state[2]>>24)] & 0x0000ff00) ^
			(_T3[(state[3]>>24)] & 0x000000ff) ^
			roundKey[R][0]
	inter[1] =
		(_T0[(state[0]>>16)&0xff] & 0xff000000) ^
			(_T1[(state[1]>>16)&0xff] & 0x00ff0000) ^
			(_T2[(state[2]>>16)&0xff] & 0x0000ff00) ^
			(_T3[(state[3]>>16)&0xff] & 0x000000ff) ^
			roundKey[R][1]
	inter[2] =
		(_T0[(state[0]>>8)&0xff] & 0xff000000) ^
			(_T1[(state[1]>>8)&0xff] & 0x00ff0000) ^
			(_T2[(state[2]>>8)&0xff] & 0x0000ff00) ^
			(_T3[(state[3]>>8)&0xff] & 0x000000ff) ^
			roundKey[R][2]
	inter[3] =
		(_T0[(state[0])&0xff] & 0xff000000) ^
			(_T1[(state[1])&0xff] & 0x00ff0000) ^
			(_T2[(state[2])&0xff] & 0x0000ff00) ^
			(_T3[(state[3])&0xff] & 0x000000ff) ^
			roundKey[R][3]
...
func (cipher *Cipher) Encrypt(dst, src []byte) {
	crypt(src, dst, cipher.roundKeyEnc, cipher.r)
}

func (cipher *Cipher) Decrypt(dst, src []byte) {
	crypt(src, dst, cipher.roundKeyDec, cipher.r)
}
