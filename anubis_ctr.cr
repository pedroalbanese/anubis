# Implementation of Anubis block cipher.
class Anubis
  ...
  def key_setup(key : String)
    n = (key.bytesize / 4).to_i
    raise "Invalid Anubis key size: #{32 * n} bits." if n < 4 || n > 10
    ...
    n.times do |i|
      kappa[i] = (
        (bytes[pos].to_u32! << 24) |
        (bytes[pos + 1].to_u32! << 16) |
        (bytes[pos + 2].to_u32! << 8) |
        bytes[pos + 3].to_u32!
      ) & 0xffffffff_u32
      pos += 4
    end

    (r_count + 1).times do |r|
      ...
      if @round_key_enc.size <= r
        @round_key_enc << Array(UInt32).new(4, 0_u32)
      end
      
      @round_key_enc[r][0] = k0
      @round_key_enc[r][1] = k1
      @round_key_enc[r][2] = k2
      @round_key_enc[r][3] = k3
      ...
      kappa[0] = ((T0[4 * r] & 0xff000000_u32) ^
                 (T1[4 * r + 1] & 0x00ff0000_u32) ^
                 (T2[4 * r + 2] & 0x0000ff00_u32) ^
                 (T3[4 * r + 3] & 0x000000ff_u32) ^
    ...
    (r_count + 1).times do
      @round_key_dec << Array(UInt32).new(4, 0_u32)
    end

    4.times do |i|
      @round_key_dec[0][i] = @round_key_enc[r_count][i]
      @round_key_dec[r_count][i] = @round_key_enc[0][i]
    end

    (1...r_count).each do |r|
      4.times do |i|
        v = @round_key_enc[r_count - r][i]
        @round_key_dec[r][i] = (T0[T4[rot(v, 24)] & 0xff] ^
  ...
  def encrypt_block(block : String) : String
    crypt(block, @round_key_enc)
  end

  def decrypt_block(block : String) : String
    crypt(block, @round_key_dec)
  end

  private def crypt(block : String, round_key : Array(Array(UInt32))) : String
    state = Array(UInt32).new(4, 0_u32)
    inter = Array(UInt32).new(4, 0_u32)
    r_count = round_key.size - 1
    ...
    (1...r_count).each do |r|
      inter[0] = (T0[rot(state[0], 24)] ^ T1[rot(state[1], 24)] ^
                  T2[rot(state[2], 24)] ^ T3[rot(state[3], 24)] ^ round_key[r][0]) & 0xffffffff_u32
      inter[1] = (T0[get_byte(state[0], 16)] ^ T1[get_byte(state[1], 16)] ^
                  T2[get_byte(state[2], 16)] ^ T3[get_byte(state[3], 16)] ^ round_key[r][1]) & 0xffffffff_u32
      inter[2] = (T0[get_byte(state[0], 8)] ^ T1[get_byte(state[1], 8)] ^
                  T2[get_byte(state[2], 8)] ^ T3[get_byte(state[3], 8)] ^ round_key[r][2]) & 0xffffffff_u32
      inter[3] = (T0[state[0] & 0xff] ^ T1[state[1] & 0xff] ^
                  T2[state[2] & 0xff] ^ T3[state[3] & 0xff] ^ round_key[r][3]) & 0xffffffff_u32
    ...
    inter[0] = ((T0[rot(state[0], 24)] & 0xff000000_u32) ^
                ...
                (T3[rot(state[3], 24)] & 0x000000ff_u32) ^ round_key[r_count][0]) & 0xffffffff_u32
    inter[1] = ((T0[get_byte(state[0], 16)] & 0xff000000_u32) ^
                (T1[get_byte(state[1], 16)] & 0x00ff0000_u32) ^
                (T2[get_byte(state[2], 16)] & 0x0000ff00_u32) ^
                (T3[get_byte(state[3], 16)] & 0x000000ff_u32) ^ round_key[r_count][1]) & 0xffffffff_u32
    inter[2] = ((T0[get_byte(state[0], 8)] & 0xff000000_u32) ^
                (T1[get_byte(state[1], 8)] & 0x00ff0000_u32) ^
                (T2[get_byte(state[2], 8)] & 0x0000ff00_u32) ^
                (T3[get_byte(state[3], 8)] & 0x000000ff_u32) ^ round_key[r_count][2]) & 0xffffffff_u32
    inter[3] = ((T0[state[0] & 0xff] & 0xff000000_u32) ^
                (T1[state[1] & 0xff] & 0x00ff0000_u32) ^
                (T2[state[2] & 0xff] & 0x0000ff00_u32) ^
                (T3[state[3] & 0xff] & 0x000000ff_u32) ^ round_key[r_count][3]) & 0xffffffff_u32

    result = IO::Memory.new
    4.times do |i|
      w = inter[i]
      result.write_byte(((w >> 24) & 0xff).to_u8)
      result.write_byte(((w >> 16) & 0xff).to_u8)
      result.write_byte(((w >> 8) & 0xff).to_u8)
      result.write_byte((w & 0xff).to_u8)
    end

    result.to_s
  end

  def ctr_mode(input : String, key : String, iv : String) : String
    key_setup(key)
    ...
    while i < input.bytesize
      block = input.byte_slice(i, Math.min(block_size, input.bytesize - i))
      keystream = encrypt_block(counter)
      
      block_bytes = block.bytes
      keystream_bytes = keystream.bytes
      
      block_bytes.each_with_index do |byte, j|
        xored = byte ^ keystream_bytes[j]
        output.write_byte(xored.to_u8)
      end
  ...
  private def increment_counter(counter : String) : String
    bytes = counter.bytes.dup
    pos = 15
    while pos >= 0
      if bytes[pos] == 255
        bytes[pos] = 0_u8
...
# CLI
if ARGV.size < 2
  puts "Uso: ./anubis <chave_hex> <iv_hex> < arquivo.txt"
  puts "Exemplo: ./anubis 00000000000000000000000000000000 00000000000000000000000000000000 < arquivo.txt"
  exit 1
end
...
begin
  ...
  puts "Erro: Falha ao converter hex para bytes: #{e.message}"
  exit 1
end

key_sizes = [16, 20, 24, 28, 32, 36, 40]
unless key_sizes.includes?(key.bytesize)
  puts "Erro: Key deve ter #{key_sizes.join(", ")} bytes (tamanho atual: #{key.bytesize})"
  puts "Isso corresponde a #{key_sizes.map { |s| s*2 }.join(", ")} caracteres hex"
  exit 1
end

if iv.bytesize != 16
  puts "Erro: IV deve ter 16 bytes (tamanho atual: #{iv.bytesize})"
  puts "Isso corresponde a 32 caracteres hex"
  exit 1
end
...
if input.empty?
  puts "Erro: Nenhum dado recebido no stdin"
  exit 1
end

begin
  anubis = Anubis.new
  resultado = anubis.ctr_mode(input, key, iv)
  STDOUT.write(resultado.to_slice)
rescue e : Exception
